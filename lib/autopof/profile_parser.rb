...
private
  ...
  def bio
    Log.info "#{self.class.name}: parsing bio"
  ...
  def name
    Log.info "#{self.class.name}: parsing name"
    name = page.css('.AboutMe').inner_text.gsub(/About\s*/, '').strip
    (name == 'Me') ? nil : name
  end

  def pof_key
    Log.info "#{self.class.name}: parsing pof_key"
    page.css('input[name="p_id"]').first.attr('value')
  end

  def username
    Log.info "#{self.class.name}: parsing username"
